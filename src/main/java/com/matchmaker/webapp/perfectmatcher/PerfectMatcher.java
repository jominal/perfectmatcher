 ...
 * Perfect Matcher is a web service that accepts as a parameter a Male name and returns as a result a Female name(mate)
 *
 */
...
public class PerfectMatcher {
	...
	/**
	 * 
	 * @param malename The male name that the user wants to check
	 * @return The female mate or error in case that the inserted name doesn't match the requested format
	 */
	@POST
	@Path("/mate") // the path of the web service (http://localhost:8080/webapp/webapi/perfectmatcher/mate)
	...
	public Response postFemaleName(@FormParam("malename") String malename) {
		...
		if (malename.endsWith("s")) {// checks if it is a Male name(for Greek names that end with an "s")
	...
	/**
	 * 
	 * @param malename The name that we want to convert
	 * @return The sum of the name letters(ASCII codes)
	 */
	@POST
	@Path("/asciiSum") // the path of the web service (http://localhost:8080/webapp/webapi/perfectmatcher/asciiSum)
	public Response postAsciiSum(@FormParam("malename") String malename) {

		int sum = Numberizer.numberizer(malename); // calculation of the sum of name letters(ASCII codes)
